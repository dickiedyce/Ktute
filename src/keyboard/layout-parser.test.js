...
`;
      ...
      // Left hand row 0 should have keys at positions with 1.0-width gaps (default)
      const leftRow0 = physical.keys.filter(k => k.hand === 'left' && k.row === 0);
      expect(leftRow0).toHaveLength(2);
      expect(leftRow0[0].col).toBe(1); // q at position 1 (after 1.0 gap)
      expect(leftRow0[1].col).toBe(3); // w at position 3 (1.0 gap + 1 key + 1.0 gap)
      
      // Right hand row 0 should have keys at positions with 1.0-width gaps
      const rightRow0 = physical.keys.filter(k => k.hand === 'right' && k.row === 0);
      expect(rightRow0).toHaveLength(2);
      expect(rightRow0[0].col).toBe(1); // e at position 1 (after 1.0 gap)
      expect(rightRow0[1].col).toBe(3); // r at position 3 (1.0 gap + 1 key + 1.0 gap)
    ...
    it('should support custom gap widths with ¦:width syntax', () => {
      const input = `
[layout:custom-gaps]
rows: 1
columns: 4,4
...
`;
      ...
      expect(physical.keys).toHaveLength(4);
      
      const leftRow0 = physical.keys.filter(k => k.hand === 'left' && k.row === 0);
      expect(leftRow0).toHaveLength(2);
      expect(leftRow0[0].col).toBe(0.25); // a after 0.25 gap
      expect(leftRow0[1].col).toBe(1.75); // b after (0.25 gap + 1 key + 0.5 gap)
      
      const rightRow0 = physical.keys.filter(k => k.hand === 'right' && k.row === 0);
      expect(rightRow0).toHaveLength(2);
      expect(rightRow0[0].col).toBe(0); // c at start
      expect(rightRow0[1].col).toBe(1.25); // d after (1 key + 0.25 gap)
    });
  });
});
